...
![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113194547079.png)
...
![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113204528201.png)

发送在远程服务器上执行`open -a Calculator.app`（打开计算器）命令的攻击Payload：

![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113205630930.png)
...
![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113211316664.png)

`ObjectInputStream`在序列化对象时是会调用`java.io.ObjectInputStream#filterCheck`->`sun.rmi.registry.RegistryImpl#registryFilter`，检测合法性：

![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113203150382.png)

当攻击者向一个实现了`JEP 290`的服务端JDK发送反序列化对象时会攻击失败并抛出：`java.io.InvalidClassException: filter status: REJECTED`异常。

![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113201005557.png)
...
![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113222313150.png)
...
![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113211451873.png)
...
![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113225456270.png)

当启动RASP后再次请求示例程序后会发现示例程序已无法正常访问，因为当RASP发现正在反序列化的类存在恶意攻击时候会立即阻断反序列化行为，如下图：

![img](https://javasec.oss-cn-hongkong.aliyuncs.com/images/image-20201113223111183.png)
