...
1. Filter：首先经过核心的过滤器，也就是通常在 `web.xml` 中配置的 filter 及 filter-mapping，这部分通常会配置 `/*` 全部的路由交给 struts2 来处理。
   <img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284287149.png" style="zoom:50%;" />
2. Interceptor-stack：执行拦截器，应用程序通常会在拦截器中实现一部分功能。也包括在 struts-core 包中 `struts-default.xml` 文件配置的默认的一些拦截器。
   <img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284293214.png" style="zoom:50%;" />
3. Action：根据访问路径，找到处理这个请求对应的 Action 控制类，通常配置在 `struts.xml` 中的 package 中。
   <img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296350.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296353.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296355.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296356.png" style="zoom:50%;" />

ActionContext 是 action 的上下文，也可以叫做 action 的数据中心，本质是一个 map，所有数据都存放在这里。

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296359.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296361.png" style="zoom:50%;" />

`findValue()` 方法调用了 `com.opensymphony.xwork2.util.TextParseUtil#translateVariables` 来解析和处理

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296363.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296365.png" style="zoom:50%;" />

然后调用 `setParameters()` 方法，循环参数 Map，首先调用 `this.acceptableName(name)` 来校验参数名是否非法，在较低版本中是判断是否包含 `#,=:`

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296367.png" style="zoom:50%;" />

在高一点的版本中是使用正则来匹配

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296370.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296372.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296373.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296375.png" style="zoom:50%;" />

以上是使用`Ognl.parseExpression()`  加 `Ognl.getValue()` 来执行的，与 `OgnlUtil.getValue()` 一致。

那使用 `OgnlUtil.setValue()`，调用会一致吗？答案是否定的。

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296377.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296383.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296384.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296387.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296389.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296393.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296395.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296396.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296398.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296400.png" style="zoom:50%;" />

此时程序会进入 struts 拦截器栈中的 `ConversionErrorInterceptor#intercept()` 方法，这个方法从 context 中获取类型转换错误的字段键值对

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296402.png" style="zoom:50%;" />

拿出这些类型转换错误的键值对，创建了一个新的 HashMap fakie，并将其储存进去，储存之前对参数值进行了处理，调用了 `getOverrideExpr()` 方法在参数值前后加了引号。

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296404.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296406.png" style="zoom:50%;" />

然后调用 `getValue()` 方法实际上就是 `Ognl.getValue()` 方法解析字符串。

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296408.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296410.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296413.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296415.png" style="zoom:50%;" />

-  `debug=xml` ：从 ServletActionContext 中获取 response 对象，把一些 context 中的内容以 xml 的格式打印出来。

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296419.png" style="zoom:50%;" />

- `debug=command&expression=`：非常清晰的漏洞调用点，如果参数 debug 是 command ，取参数 expression 的值，并调用 `stack.findValue()` 进行解析。

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296421.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296423.png" style="zoom:50%;" />

模板引入了 struts/webconsole.html ，我们也可以直接访问这个路径来访问这个页面

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296426.png" style="zoom:50%;" />

<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296428.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296429.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296431.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296434.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296436.png" style="zoom:50%;" />
...
<img src="https://javasec.oss-cn-hongkong.aliyuncs.com/images/1625284296438.png" style="zoom:50%;" />
